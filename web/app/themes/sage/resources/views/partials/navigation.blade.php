@section('toggle-open')
  <button nav-toggle="open" toggle-target="nav-overlay" class="w-full inline-flex items-center justify-end pr-0 justify-end text-white transition transition-opacity duration-500 ease-in-out border-0 rounded focus:outline-none">
    @svg('bars', 'menu-icon menu-icon-open w-12 justify-end fill-current blend-exclusion transition transition-opacity duration-500 ease-in-out stroke-white text-white', ['width' => '1.5rem', 'height' => '1.5rem'])
  </button>
@endsection
...
<header class="fixed z-50 flex w-full overflow-hidden transition transition-opacity duration-500 ease-in-out bg-opacity-75 bg-black nav">
...
<div class="fixed top-0 bottom-0 left-0 right-0 z-40 flex flex-col h-screen w-full h-0 overflow-hidden transition transition-opacity duration-500 ease-in-out bg-black opacity-0 nav-overlay bg-black">
        ...
        @if ($navigation->about)
          ...
          <nav class="relative flex flex-col flex-grow-0 mx-16 mx-auto text-center justify-middle" style="top: -0.75rem">
            @foreach ($navigation->about as $item)
              <a hoverfx fx-duration="800" fx-on-scale="1.1" fx-off-scale="1" fx-on-translate-y="-3px" fx-off-translate-y="0px" class="flex-grow-0 text-xl lg:text-3xl font-bold text-white uppercase md:text-2xl font-display nav-item" href="{!! $item->url !!}">{!! $item->label !!}</a>
            @endforeach
          </nav>
        @endif
