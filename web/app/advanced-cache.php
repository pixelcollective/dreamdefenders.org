...
if ( version_compare( phpversion(), '5.6' ) >= 0 ) {
	...
	spl_autoload_register(
		function( $class ) {
			$rocket_path    = '/srv/www/dreamdefenders.org/current/web/app/plugins/wp-rocket/';
			$rocket_classes = [
	...
	$rocket_config_class = new \WP_Rocket\Buffer\Config(
		[
			'config_dir_path' => '/srv/www/dreamdefenders.org/current/web/app/wp-rocket-config/',
		]
	);
	...
	( new \WP_Rocket\Buffer\Cache(
		...
		$rocket_config_class,
		[
			'cache_dir_path' => '/srv/www/dreamdefenders.org/current/web/app/cache/wp-rocket/',
		]
	) )->maybe_init_process();
} else {
